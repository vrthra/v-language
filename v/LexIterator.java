...
public class LexIterator extends QuoteIterator {
    ...
    private Term compound(Term<Character> open) {
        ...
        while(true) {
            Term t = lex_next();
            if (t == null)
                throw new VSynException("err:lex:close", "Need " + Lexer.closeCompound(open.val));
        ...
        CmdQuote cq = new CmdQuote(local, open.val);
        return new Term<Quote>(Type.TQuote, cq);
    }

    public void remove() {
        throw new VSynException("err:internal:lex:remove", "remove not supported.");
    }
}
